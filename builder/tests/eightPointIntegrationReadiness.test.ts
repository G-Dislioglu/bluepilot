...
test('eight point readiness covers all requested wiring areas without side effects', () => {
  ...
  assert.equal(readiness.summary.wiredContractOnly, 4);
  assert.equal(readiness.summary.lockedForLaterActivation, 0);
  ...
  assert.equal(readiness.points.find((point) => point.id === 'merge_release_readiness')?.status, 'wired_contract_only');
  assert.equal(readiness.points.find((point) => point.id === 'merge_release_readiness')?.endpoint, '/probe/merge-release-readiness-contract');
