import type { RuntimePatchPermitConsumeExecutionReceiptRecordPreflight } from './runtimePatchPermitConsumeExecutionReceiptRecordPreflight.js';

export type RuntimePatchPermitConsumeExecutionReceiptRecordAuthorityStatus = 'ready' | 'review_required' | 'blocked';

export interface RuntimePatchPermitConsumeExecutionReceiptRecordAuthorityInput {
  preflight: RuntimePatchPermitConsumeExecutionReceiptRecordPreflight;
  receiptRecordAuthorityId?: string;
  authorizedByRef?: string;
  expiresAtRef?: string;
}

export interface RuntimePatchPermitConsumeExecutionReceiptRecordAuthority {
  status: RuntimePatchPermitConsumeExecutionReceiptRecordAuthorityStatus;
...
export function authorizeRuntimePatchPermitConsumeExecutionReceiptRecord(
  input: RuntimePatchPermitConsumeExecutionReceiptRecordAuthorityInput,
): RuntimePatchPermitConsumeExecutionReceiptRecordAuthority {
  ...
  if (input.preflight.status === 'blocked') {
    blockers.push(...input.preflight.blockers.map((blocker) => `runtime_patch_permit_consume_execution_receipt_record_authority.preflight_blocked:${blocker}`));
  }
  if (input.preflight.status === 'review_required') {
    reviewItems.push(...input.preflight.reviewItems.map((item) => `runtime_patch_permit_consume_execution_receipt_record_authority.preflight_review_required:${item}`));
  }
  if (!input.preflight.consumeExecutionReceiptRecordPreflightAllowed) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_authority.preflight_not_allowed');
  }
  if (!input.preflight.consumeExecutionReceiptAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_authority.receipt_must_be_authorized');
  }
  if (!input.preflight.consumeExecutionAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_authority.execution_must_be_authorized');
  }
  if (
    ...
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_authority.runtime_action_gates_must_stay_closed');
  }
  if (!receiptRecordAuthorityId) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record_authority.receipt_record_authority_id_required');
  }
  if (!authorizedByRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record_authority.authorized_by_ref_required');
  }
  if (!expiresAtRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record_authority.expires_at_ref_required');
  }

  const status: RuntimePatchPermitConsumeExecutionReceiptRecordAuthorityStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_runtime_patch_permit_consume_execution_receipt_record']
      : status === 'review_required'
        ? ['complete_runtime_patch_permit_consume_execution_receipt_record_authority_review']
        : ['resolve_runtime_patch_permit_consume_execution_receipt_record_authority_blockers'],
  };
}
