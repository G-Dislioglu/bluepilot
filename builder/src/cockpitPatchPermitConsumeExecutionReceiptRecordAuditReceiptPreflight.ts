import type { CockpitPatchPermitConsumeExecutionReceiptRecordAudit } from './cockpitPatchPermitConsumeExecutionReceiptRecordAudit.js';

export type CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflightStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflightInput {
  audit: CockpitPatchPermitConsumeExecutionReceiptRecordAudit;
  auditReceiptRef?: string;
  auditReceiptRecorderRef?: string;
  auditReceiptPolicyRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflight {
  status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflightStatus;
  ...
  evidenceRefs: string[];
  auditReceiptPlan: {
    kind: 'cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight';
...
export function preflightCockpitPatchPermitConsumeExecutionReceiptRecordAuditReceipt(
  input: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflightInput,
): CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflight {
  ...
  if (input.audit.status === 'blocked') {
    blockers.push(...input.audit.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_blocked:${blocker}`));
  }
  if (input.audit.status === 'review_required') {
    reviewItems.push(...input.audit.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_review_required:${item}`));
  }
  if (!input.audit.consumeExecutionReceiptRecordAudited || !input.audit.consumeExecutionReceiptRecordAuditAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_not_complete');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.action_gates_must_stay_closed');
  }
  if (!auditReceiptRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_receipt_ref_required');
  }
  if (!auditReceiptRecorderRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_receipt_recorder_ref_required');
  }
  if (!auditReceiptPolicyRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight.audit_receipt_policy_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptPreflightStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_authority']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight_review']
        : ['resolve_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_preflight_blockers'],
  };
}
