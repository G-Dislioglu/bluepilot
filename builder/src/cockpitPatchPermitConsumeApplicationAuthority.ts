import type { CockpitPatchPermitConsumeApplicationPreflight } from './cockpitPatchPermitConsumeApplicationPreflight.js';

export type CockpitPatchPermitConsumeApplicationAuthorityStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeApplicationAuthorityInput {
  preflight: CockpitPatchPermitConsumeApplicationPreflight;
  applicationAuthorityId?: string;
  authorizedByRef?: string;
  expiresAtRef?: string;
}

export interface CockpitPatchPermitConsumeApplicationAuthority {
  status: CockpitPatchPermitConsumeApplicationAuthorityStatus;
...
export function authorizeCockpitPatchPermitConsumeApplication(
  input: CockpitPatchPermitConsumeApplicationAuthorityInput,
  ...
  if (input.preflight.status === 'blocked') {
    blockers.push(...input.preflight.blockers.map((blocker) => `cockpit_patch_permit_consume_application_authority.preflight_blocked:${blocker}`));
  }
  if (input.preflight.status === 'review_required') {
    reviewItems.push(...input.preflight.reviewItems.map((item) => `cockpit_patch_permit_consume_application_authority.preflight_review_required:${item}`));
  }
  if (!input.preflight.consumeApplicationPreflightAllowed) {
    blockers.push('cockpit_patch_permit_consume_application_authority.preflight_not_allowed');
  }
  if (!input.preflight.permitConsumeAuthorized) {
    blockers.push('cockpit_patch_permit_consume_application_authority.consume_must_be_authorized');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_application_authority.action_gates_must_stay_closed');
  }
  if (!applicationAuthorityId) {
    reviewItems.push('cockpit_patch_permit_consume_application_authority.application_authority_id_required');
  }
  if (!authorizedByRef) {
    reviewItems.push('cockpit_patch_permit_consume_application_authority.authorized_by_ref_required');
  }
  if (!expiresAtRef) {
    reviewItems.push('cockpit_patch_permit_consume_application_authority.expires_at_ref_required');
  }

  const status: CockpitPatchPermitConsumeApplicationAuthorityStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_preflight']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_application_authority_review']
        : ['resolve_cockpit_patch_permit_consume_application_authority_blockers'],
  };
}
