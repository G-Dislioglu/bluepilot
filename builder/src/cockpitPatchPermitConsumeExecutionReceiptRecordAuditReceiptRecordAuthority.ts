import type { CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflight } from './cockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflight.js';

export type CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthorityStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthorityInput {
  preflight: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflight;
  auditReceiptRecordAuthorityId?: string;
  authorizedByRef?: string;
  expiresAtRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthority {
  status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthorityStatus;
  ...
  authorizedAuditReceiptRecord: {
    kind: 'cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority';
...
export function authorizeCockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecord(
  input: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthorityInput,
): CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthority {
  ...
  if (input.preflight.status === 'blocked') {
    blockers.push(...input.preflight.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.preflight_blocked:${blocker}`));
  }
  if (input.preflight.status === 'review_required') {
    reviewItems.push(...input.preflight.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.preflight_review_required:${item}`));
  }
  if (!input.preflight.consumeExecutionReceiptRecordAuditReceiptRecordPreflightAllowed) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.preflight_not_allowed');
  }
  if (!input.preflight.consumeExecutionReceiptRecordAuditReceiptRecorded || !input.preflight.consumeExecutionReceiptRecordAuditReceiptAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.audit_receipt_record_must_be_preflighted');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.action_gates_must_stay_closed');
  }
  if (!auditReceiptRecordAuthorityId) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.audit_receipt_record_authority_id_required');
  }
  if (!authorizedByRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.authorized_by_ref_required');
  }
  if (!expiresAtRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority.expires_at_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthorityStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority_review']
        : ['resolve_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority_blockers'],
  };
}
