import type { RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditAuthority } from './runtimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditAuthority.js';

export type RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditStatus = 'audited' | 'review_required' | 'blocked';

export interface RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditInput {
  authority: RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditAuthority;
  auditedAtRef?: string;
  auditEvidenceRef?: string;
}

export interface RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAudit {
  status: RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditStatus;
  ...
  recordedAuditReceiptRecordAudit: {
    kind: 'runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit';
...
export function recordRuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAudit(
  input: RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditInput,
): RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAudit {
  ...
  if (input.authority.status === 'blocked') {
    blockers.push(...input.authority.blockers.map((blocker) => `runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.authority_blocked:${blocker}`));
  }
  if (input.authority.status === 'review_required') {
    reviewItems.push(...input.authority.reviewItems.map((item) => `runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.authority_review_required:${item}`));
  }
  if (!input.authority.consumeExecutionReceiptRecordAuditReceiptRecordAuditAuthorityAllowed || !input.authority.consumeExecutionReceiptRecordAuditReceiptRecordAuditAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.audit_authority_not_allowed');
  }
  if (!input.authority.consumeExecutionReceiptRecordAuditReceiptRecordRecorded || !input.authority.consumeExecutionReceiptRecordAuditReceiptRecordAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.audit_receipt_record_must_be_recorded');
  }
  if (
    ...
    blockers.push('runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.runtime_action_gates_must_stay_closed');
  }
  if (!auditedAtRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.audited_at_ref_required');
  }
  if (!auditEvidenceRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit.audit_evidence_ref_required');
  }

  const status: RuntimePatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuditStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'audited'
      ? ['open_task_lock_for_runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit_receipt_preflight']
      : status === 'review_required'
        ? ['complete_runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit_review']
        : ['resolve_runtime_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit_blockers'],
  };
}
