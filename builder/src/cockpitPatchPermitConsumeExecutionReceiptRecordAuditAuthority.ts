import type { CockpitPatchPermitConsumeExecutionReceiptRecordAuditPreflight } from './cockpitPatchPermitConsumeExecutionReceiptRecordAuditPreflight.js';

export type CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthorityStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthorityInput {
  preflight: CockpitPatchPermitConsumeExecutionReceiptRecordAuditPreflight;
  auditAuthorityId?: string;
  authorizedByRef?: string;
  expiresAtRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthority {
  status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthorityStatus;
...
export function authorizeCockpitPatchPermitConsumeExecutionReceiptRecordAudit(
  input: CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthorityInput,
  ...
  if (input.preflight.status === 'blocked') {
    blockers.push(...input.preflight.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_receipt_record_audit_authority.preflight_blocked:${blocker}`));
  }
  if (input.preflight.status === 'review_required') {
    reviewItems.push(...input.preflight.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_receipt_record_audit_authority.preflight_review_required:${item}`));
  }
  if (!input.preflight.consumeExecutionReceiptRecordAuditPreflightAllowed) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.preflight_not_allowed');
  }
  if (!input.preflight.consumeExecutionReceiptRecorded || !input.preflight.executionReceiptRecorded) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.record_must_be_recorded');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.action_gates_must_stay_closed');
  }
  if (!auditAuthorityId) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.audit_authority_id_required');
  }
  if (!authorizedByRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.authorized_by_ref_required');
  }
  if (!expiresAtRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_authority.expires_at_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditAuthorityStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_receipt_record_audit']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_receipt_record_audit_authority_review']
        : ['resolve_cockpit_patch_permit_consume_execution_receipt_record_audit_authority_blockers'],
  };
}
