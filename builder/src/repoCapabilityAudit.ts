...
const ADOPTION_CANDIDATES: BluepilotAdoptionCandidate[] = [
  ...
  },
  {
    ...
    firstSafeWiring: 'GET /probe/patrol-visual-coverage exposes patrolVisualCoverage-style planning before mounting UI routes.',
    blockedSideEffects: ['screenshotCapture', 'taskCreation', 'fileWrites', 'runtimeExecution'],
    nextImplementationSlice: 'mount read-only patrol summary after operator review',
  },
  {
    ...
    firstSafeWiring: 'GET /probe/bpk-execution-ledger maps BPK authority and receipt artifacts into a read-only ledger summary.',
    blockedSideEffects: ['permitConsumption', 'patchApply', 'externalAction', 'merge'],
    nextImplementationSlice: 'wire operator ledger UI',
  },
  {
    ...
    firstSafeWiring: 'GET /probe/repo-mutation-kill-switch exposes locked status; no toggle persistence in the first slice.',
    blockedSideEffects: ['fileWrites', 'durablePersistence', 'githubWrites'],
    nextImplementationSlice: 'add operator-visible toggle contract without persistence',
  },
  {
    ...
    firstSafeWiring: 'GET /probe/aicos-permission-map exposes static permission hints, not live registry mutation.',
    blockedSideEffects: ['registryWrites', 'runtimeActivation', 'providerCalls'],
    nextImplementationSlice: 'bind static permission ids to task locks after registry review',
  },
  {
...
export function buildRepoCapabilityAudit(now = new Date()): RepoCapabilityAudit {
  return {
    ...
    recommendedNextSlice: 'bluepilot-operator-ledger-ui-v0.1',
  };
}
