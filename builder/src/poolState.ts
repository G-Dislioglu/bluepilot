...
function isProviderConfigured(provider: string): boolean {
  ...
  return envKey ? Boolean(process.env[envKey]?.trim()) : true;
}
...
export const POOL_MODEL_CATALOG: Record<string, PoolModelCatalogEntry> = {
  ...
  grok: {
    ...
    speed: 'fast',
    color: '#ef4444',
    pools: ['maya', 'council', 'scout'],
    visionCapable: true,
    supportsMultiImage: true,
    supportsWebResearch: false,
    recommendedVisualRoles: ['ui_review', 'ocr_and_label_check', 'operator_confusion'],
  },
  deepseek: {
    id: 'deepseek',
  ...
  'glm-turbo': {
    ...
    speed: 'fast',
    color: '#34d399',
    pools: ['maya', 'council', 'distiller', 'worker'],
  },
  'deepseek-pro': {
    id: 'deepseek-pro',
  ...
  'qwen-vl': {
    ...
    deprecated: true,
    deprecatedReason: 'Legacy technical Vision slot. Use current Council id qwen unless Builder explicitly re-validates a Vision-capable replacement.',
  },
  'glm-46v': {
    id: 'glm-46v',
    label: 'GLM 4.6V',
    ...
    deprecated: true,
    deprecatedReason: 'Legacy technical Vision slot. Not part of the current visible Council model contract.',
  },
  'glm-5v': {
    id: 'glm-5v',
    label: 'GLM 5V Turbo',
    ...
    deprecated: true,
    deprecatedReason: 'Legacy technical Vision slot. Use current Council id glm-turbo unless Builder explicitly re-validates a Vision-capable replacement.',
  },
  'deepseek-scout': {
    id: 'deepseek-scout',
    label: 'DeepSeek V4 Flash',
...
function getActivePoolsForModel(modelId: string): PoolType[] {
  return (Object.keys(activePools) as PoolType[]).filter((pool) => activePools[pool].includes(modelId));
}

export function isPoolModelExecutable(id: string): boolean {
  const entry = POOL_MODEL_CATALOG[id];
  if (!entry || entry.deprecated === true) return false;
  return isProviderConfigured(entry.provider);
}

export function getProviderHealthSnapshot(): ProviderHealthSnapshot {
  const providers = [...new Set(Object.values(POOL_MODEL_CATALOG).map((entry) => entry.provider))].sort();
  const models = Object.values(POOL_MODEL_CATALOG).map<ModelHealthEntry>((entry) => {
    const envKey = PROVIDER_ENV_KEYS[entry.provider] ?? null;
    const configured = isProviderConfigured(entry.provider);
    const blocked = entry.deprecated === true || !configured;
    return {
      id: entry.id,
      provider: entry.provider,
      status: blocked ? 'blocked' : 'ready',
      reason: entry.deprecated === true
        ? 'deprecated'
        : !configured
          ? `${envKey ?? 'provider_key'} missing`
          : null,
    };
  });
  const blockedModelIds = models.filter((model) => model.status === 'blocked').map((model) => model.id);
  const activeBlockedModelIds = blockedModelIds.filter((id) => getActivePoolsForModel(id).length > 0);

  const providerEntries = providers.map<ProviderHealthEntry>((provider) => {
    const envKey = PROVIDER_ENV_KEYS[provider] ?? null;
    const configured = isProviderConfigured(provider);
    const affectedModelIds = Object.values(POOL_MODEL_CATALOG)
      .filter((entry) => entry.provider === provider && !configured)
      .map((entry) => entry.id);
    const activeInPools = [...new Set(affectedModelIds.flatMap((id) => getActivePoolsForModel(id)))];
    return {
      provider,
      status: configured ? 'ready' : 'missing_key',
      requiredEnvKey: envKey,
      configured,
      affectedModelIds,
      activeInPools,
      summary: configured
        ? `${provider} ready`
        : `${provider} blocked: ${envKey ?? 'provider key'} missing`,
    };
  });

  const missingProviders = providerEntries.filter((entry) => entry.status === 'missing_key');
  return {
    providers: providerEntries,
    models,
    blockedModelIds,
    activeBlockedModelIds,
    openRouterReady: providerEntries.find((entry) => entry.provider === 'openrouter')?.status === 'ready',
    summary: missingProviders.length === 0
      ? 'Alle Provider-Keys fuer sichtbare Modelle sind gesetzt.'
      : `Fehlende Provider: ${missingProviders.map((entry) => entry.provider).join(', ')}.`,
  };
}

const activePools: PoolConfig = {
  maya: ['glm51'],
  council: ['glm51', 'grok', 'deepseek'],
  worker: ['glm-turbo', 'glm51', 'minimax', 'kimi', 'mimo', 'qwen'],
  scout: ['grok', 'deepseek-scout', 'glm-flash', 'gemini-flash'],
  distiller: ['glm-flash'],
};
...
export function getVisionCapableModels(): PoolModelCatalogEntry[] {
  return Object.values(POOL_MODEL_CATALOG).filter((entry) => entry.visionCapable === true && entry.deprecated !== true);
}

export function getDeprecatedVisionModels(): PoolModelCatalogEntry[] {
  return Object.values(POOL_MODEL_CATALOG).filter((entry) => entry.visionCapable === true && entry.deprecated === true);
}

export function getExecutableVisualReviewModel(id: string): PoolModelCatalogEntry | null {
  const entry = getPoolModelCatalogEntry(id);
  if (!entry || entry.visionCapable !== true || !isPoolModelExecutable(id)) {
    return null;
  }
  return entry;
}

export function getExecutableVisualCouncilModel(id: string): PoolModelCatalogEntry | null {
  const entry = getPoolModelCatalogEntry(id);
  if (!entry || !isPoolModelExecutable(id) || !POOL_AVAILABLE_MODELS.council.includes(id)) {
    return null;
  }
  return entry;
}
...
export function resolveModelById(id: string): ResolvedModel | null {
  ...
  if (!entry || !isPoolModelExecutable(id)) {
    return null;
  }
...
export function pickFromPool(pool: PoolType, preferStrong = true): ResolvedModel | null {
  const ids = activePools[pool].filter(isPoolModelExecutable);
