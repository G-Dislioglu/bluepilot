import type { CockpitPatchPermitConsumeApplicationAuthority } from './cockpitPatchPermitConsumeApplicationAuthority.js';

export type CockpitPatchPermitConsumeExecutionPreflightStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionPreflightInput {
  authority: CockpitPatchPermitConsumeApplicationAuthority;
  executionPreflightRef?: string;
  executorRef?: string;
  executionPolicyRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionPreflight {
  status: CockpitPatchPermitConsumeExecutionPreflightStatus;
...
export function preflightCockpitPatchPermitConsumeExecution(
  input: CockpitPatchPermitConsumeExecutionPreflightInput,
  ...
  if (input.authority.status === 'blocked') {
    blockers.push(...input.authority.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_preflight.authority_blocked:${blocker}`));
  }
  if (input.authority.status === 'review_required') {
    reviewItems.push(...input.authority.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_preflight.authority_review_required:${item}`));
  }
  if (!input.authority.consumeApplicationAuthorityAllowed || !input.authority.consumeApplicationAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_preflight.application_authority_not_allowed');
  }
  if (!input.authority.permitConsumeAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_preflight.consume_must_be_authorized');
  }
  if (!input.authority.permitIssued) {
    blockers.push('cockpit_patch_permit_consume_execution_preflight.permit_must_be_issued');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_preflight.action_gates_must_stay_closed');
  }
  if (!executionPreflightRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_preflight.execution_preflight_ref_required');
  }
  if (!executorRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_preflight.executor_ref_required');
  }
  if (!executionPolicyRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_preflight.execution_policy_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionPreflightStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_authority']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_preflight_review']
        : ['resolve_cockpit_patch_permit_consume_execution_preflight_blockers'],
  };
}
