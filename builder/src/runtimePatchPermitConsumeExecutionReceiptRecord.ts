import type { RuntimePatchPermitConsumeExecutionReceiptRecordAuthority } from './runtimePatchPermitConsumeExecutionReceiptRecordAuthority.js';

export type RuntimePatchPermitConsumeExecutionReceiptRecordStatus = 'recorded' | 'review_required' | 'blocked';

export interface RuntimePatchPermitConsumeExecutionReceiptRecordInput {
  authority: RuntimePatchPermitConsumeExecutionReceiptRecordAuthority;
  recordedAtRef?: string;
  receiptRecordEvidenceRef?: string;
}

export interface RuntimePatchPermitConsumeExecutionReceiptRecord {
  status: RuntimePatchPermitConsumeExecutionReceiptRecordStatus;
...
export function recordRuntimePatchPermitConsumeExecutionReceipt(
  input: RuntimePatchPermitConsumeExecutionReceiptRecordInput,
): RuntimePatchPermitConsumeExecutionReceiptRecord {
  ...
  if (input.authority.status === 'blocked') {
    blockers.push(...input.authority.blockers.map((blocker) => `runtime_patch_permit_consume_execution_receipt_record.authority_blocked:${blocker}`));
  }
  if (input.authority.status === 'review_required') {
    reviewItems.push(...input.authority.reviewItems.map((item) => `runtime_patch_permit_consume_execution_receipt_record.authority_review_required:${item}`));
  }
  if (!input.authority.consumeExecutionReceiptRecordAuthorityAllowed || !input.authority.consumeExecutionReceiptRecordAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record.record_authority_not_allowed');
  }
  if (!input.authority.consumeExecutionReceiptAuthorized) {
    blockers.push('runtime_patch_permit_consume_execution_receipt_record.receipt_must_be_authorized');
  }
  if (
    ...
    blockers.push('runtime_patch_permit_consume_execution_receipt_record.runtime_action_gates_must_stay_closed');
  }
  if (!recordedAtRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record.recorded_at_ref_required');
  }
  if (!receiptRecordEvidenceRef) {
    reviewItems.push('runtime_patch_permit_consume_execution_receipt_record.receipt_record_evidence_ref_required');
  }

  const status: RuntimePatchPermitConsumeExecutionReceiptRecordStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'recorded'
      ? ['open_task_lock_for_runtime_patch_permit_consume_execution_receipt_record_audit_preflight']
      : status === 'review_required'
        ? ['complete_runtime_patch_permit_consume_execution_receipt_record_review']
        : ['resolve_runtime_patch_permit_consume_execution_receipt_record_blockers'],
  };
}
