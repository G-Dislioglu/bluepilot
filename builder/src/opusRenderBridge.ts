...
export type RenderLogType = 'build' | 'app' | 'deploy';

export async function listRecentLogs(limit = 50, type: RenderLogType = 'build'): Promise<{
  logs?: RenderLogEntry[];
  error?: string;
  type?: RenderLogType;
}> {
...
export async function listRecentBuildLogs(limit = 50): Promise<{
  ...
  return listRecentLogs(limit, 'build');
}
