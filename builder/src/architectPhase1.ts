...
function pushSoftLimitWarning(
  ...
  warnings.push(
    `${label} exceeds the Phase-1 soft limit of ${limit} characters (${length}); continuing with state-preserving non-blocking dispatch.`,
  );
}
...
export async function assembleArchitectInstruction(
  ...
  pushSoftLimitWarning(warnings, 'User instruction', instruction.length, USER_INSTRUCTION_SOFT_LIMIT);
  ...
  pushSoftLimitWarning(warnings, 'Final dispatch instruction', finalInstruction.length, FINAL_DISPATCH_SOFT_LIMIT);
