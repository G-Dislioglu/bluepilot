import type { CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceipt } from './cockpitPatchPermitConsumeExecutionReceiptRecordAuditReceipt.js';

export type CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflightStatus = 'ready' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflightInput {
  auditReceipt: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceipt;
  auditReceiptRecordRef?: string;
  auditReceiptRecorderRef?: string;
  auditReceiptRecordPolicyRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflight {
  status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflightStatus;
...
export function preflightCockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecord(
  input: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflightInput,
): CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflight {
  ...
  if (input.auditReceipt.status === 'blocked') {
    blockers.push(...input.auditReceipt.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_blocked:${blocker}`));
  }
  if (input.auditReceipt.status === 'review_required') {
    reviewItems.push(...input.auditReceipt.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_review_required:${item}`));
  }
  if (!input.auditReceipt.consumeExecutionReceiptRecordAuditReceiptRecorded || !input.auditReceipt.consumeExecutionReceiptRecordAuditReceiptAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_not_recorded');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.action_gates_must_stay_closed');
  }
  if (!auditReceiptRecordRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_record_ref_required');
  }
  if (!auditReceiptRecorderRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_recorder_ref_required');
  }
  if (!auditReceiptRecordPolicyRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight.audit_receipt_record_policy_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordPreflightStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'ready'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_authority']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight_review']
        : ['resolve_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_preflight_blockers'],
  };
}
