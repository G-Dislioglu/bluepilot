import type { CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthority } from './cockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthority.js';

export type CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordStatus = 'recorded' | 'review_required' | 'blocked';

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordInput {
  authority: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordAuthority;
  recordedAtRef?: string;
  auditReceiptRecordEvidenceRef?: string;
}

export interface CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecord {
  status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordStatus;
...
export function recordCockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecord(
  input: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordInput,
): CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecord {
  ...
  if (input.authority.status === 'blocked') {
    blockers.push(...input.authority.blockers.map((blocker) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.authority_blocked:${blocker}`));
  }
  if (input.authority.status === 'review_required') {
    reviewItems.push(...input.authority.reviewItems.map((item) => `cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.authority_review_required:${item}`));
  }
  if (!input.authority.consumeExecutionReceiptRecordAuditReceiptRecordAuthorityAllowed || !input.authority.consumeExecutionReceiptRecordAuditReceiptRecordAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.record_authority_not_allowed');
  }
  if (!input.authority.consumeExecutionReceiptRecordAuditReceiptRecorded || !input.authority.consumeExecutionReceiptRecordAuditReceiptAuthorized) {
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.audit_receipt_must_be_recorded');
  }
  if (
    ...
    blockers.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.action_gates_must_stay_closed');
  }
  if (!recordedAtRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.recorded_at_ref_required');
  }
  if (!auditReceiptRecordEvidenceRef) {
    reviewItems.push('cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record.audit_receipt_record_evidence_ref_required');
  }

  const status: CockpitPatchPermitConsumeExecutionReceiptRecordAuditReceiptRecordStatus = blockers.length > 0
    ? 'blocked'
  ...
  return {
    ...
    nextActions: status === 'recorded'
      ? ['open_task_lock_for_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_audit_preflight']
      : status === 'review_required'
        ? ['complete_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_review']
        : ['resolve_cockpit_patch_permit_consume_execution_receipt_record_audit_receipt_record_blockers'],
  };
}
