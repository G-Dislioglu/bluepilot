...
- Aktueller Arbeitsbranch: `bluepilot-deploy-readiness-stack`.
...
- BP-150 retired diese permitlose Seitentuer wieder: `/probe/sandbox-write`
  antwortet nur noch HTTP 410. Der Content-Write-Koerper wurde aus
  `builder/src/sandboxWrite.ts` entfernt; der bewiesene SmartPush-/Permit-Pfad
  bleibt der einzige dokumentierte Schreibkorridor.
...
- BPK-002 bleibt historischer Permit-Generalization-Stand. Im Deploy-Readiness-Merge gewinnt
  BP-150: der aktive `/probe/sandbox-write`-Handler bleibt HTTP 410 und oeffnet keinen Write.
...
- BP-149: Freie Maya-Sandbox-Steuerung Teil 1; neuer `POST /probe/sandbox-write`
  ...
  unberuehrt.
- BP-150: Sandbox write retire; `POST /probe/sandbox-write` bleibt als klares Runtime-Signal
  erhalten, ist aber nur noch HTTP 410. Die permitlose GitHub-Content-Write-Implementierung und
  die Nutzung von `BLUEPILOT_SANDBOX_PERMIT_WRITE_ENABLED` in `builder/src` sind entfernt.
- BPK-002: Permit generalization bleibt historischer Zwischenstand. Fuer den Deploy-Readiness-
  Merge gilt BP-150 als aktiver Stand: kein `/probe/sandbox-write`-Write, keine Delete/Undo-
  Ausfuehrung, keine GitHub-Content-Write-Seitentuer.
