...
- Aktueller BPK-Arbeitsbranch: `bpk-159-162-consume-execution-receipt-record-audit-preflight`.
...
Nach BPK-162 ist das Permit-/Approved-Action-Consume-Execution-Receipt-Record-Audit-Preflight Bundle abgeschlossen, wenn:

- die Review-Packets fuer BPK-159 bis BPK-162 existieren,
- die vier fokussierten Consume-Execution-Receipt-Record-Audit-Preflight-Tests und `npm run typecheck` in `builder/` gruen
  sind,
- `node tools/verify-task-lock.cjs BPK-159 --verify` bis BPK-162 gruen sind,
