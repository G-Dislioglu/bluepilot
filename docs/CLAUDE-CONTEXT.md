...
- Aktueller BPK-Arbeitsbranch: `bpk-163-166-consume-execution-receipt-record-audit-authority`.
...
Nach BPK-166 ist das Permit-/Approved-Action-Consume-Execution-Receipt-Record-Audit-Authority Bundle abgeschlossen, wenn:

- die Review-Packets fuer BPK-163 bis BPK-166 existieren,
- die vier fokussierten Consume-Execution-Receipt-Record-Audit-Authority-Tests und `npm run typecheck` in `builder/` gruen
  sind,
- `node tools/verify-task-lock.cjs BPK-163 --verify` bis BPK-166 gruen sind,
...
1. Cockpit Patch Permit Consume Execution Receipt Record Audit.
2. Memory Cache Audit Export Permit Consume Execution Receipt Record Audit.
3. Runtime Patch Permit Consume Execution Receipt Record Audit.
4. Release Governance Approved Action Consume Execution Receipt Record Audit.
